...
void Font::LoadFromFile(wchar const *filename)
{
	mName = ToLower(wstring(filename));

	wstring buffer;
	xml_doc *doc = LoadUTF8XMLFile(Format(TEXT("data\\%s.bitmapfont"), filename).c_str(), buffer);
	...
	if(root != null)
	{
		...
		if(layersNode != null)
		{
			...
			if(mLayerCount != 0)
			{
				...
				for(int i=0; i<mPageCount; ++i)
				{
					mPages[i] = Texture::Load(Format(TEXT("data\\%s%d.dds"), filename, i).c_str());
				}
...
bool Font::DrawChar(int layer, Vec2 &cursor, SpriteList &spriteList, wchar c, Color color)
{
	...
	if(i != mGlyphMap.end())
	{
		...
		if(layer < glyph.imageTableSize)
		{
			...
			spriteList.AddSpriteEx(cursor + graphic.drawOffset, Vec2(graphic.size), graphic.position, graphic.position + graphic.size, color);
		}
...
{
	Vec2 dummyOffset;
	Vec2 &off = (offset == null) ? dummyOffset : *offset;
	off.Set(0,0);
	Vec2 mmax(0.0f, (float)mHeight);
...
void Font::DrawString(SpriteList &spriteList, string const &text, Vec2 &pos, Font::HorizontalAlign horizAlign, Font::VerticalAlign vertAlign)
{
...
void Font::DrawString(SpriteList &spriteList, char const *text, Vec2 &pos, Font::HorizontalAlign horizAlign, Font::VerticalAlign vertAlign)
{
	assert(mSpriteList == null);
...
void Font::DrawStringMultiple(string const &text, Vec2 &pos, Font::HorizontalAlign horizAlign, Font::VerticalAlign vertAlign)
{
...
void Font::DrawStringMultiple(char const *text, Vec2 &pos, Font::HorizontalAlign horizAlign, Font::VerticalAlign vertAlign)
{
...
void Font::DrawStringInternal(SpriteList *spriteList, char const *text, Vec2 &pos, Font::HorizontalAlign horizAlign, Font::VerticalAlign vertAlign)
{
	Vec2 drawOffset;
	Vec2 offset;
	Vec2 stringSize;
